...
▼
┌─────────────────────────────────────────┐
│ Robust Parser + Groq LLM (Query Parsing)│ ← Extracts genre, year, exclusions, etc.
└─────────────────────────────────────────┘
│
▼
┌─────────────────────────────────────────┐
...
| Feature | Description |
|---------|-------------|
| **Natural Language Understanding** | Robust rule‑based parser extracts filters like genre, year, "not", and "similar to"; optionally enhanced by Groq Llama-3 |
| **Semantic Search** | Sentence‑Transformers (`all-MiniLM-L6-v2`) + FAISS index for sub‑second retrieval |
| **Explainable AI** | Every recommendation includes a concise sentence explaining the match (AI‑generated, with template fallback) |
| **Hybrid Scoring** | Combines semantic similarity, average rating, and log‑scaled popularity into a single score; user‑adjustable weights |
| **Interactive UI** | Streamlit interface with sliders to adjust ranking weights in real‑time |
| **Fault‑tolerant** | Graceful fallback to template explanations when Groq is unavailable; rule‑based parser always active |
...
├── benchmark/
│   ├── test_queries.json       # Ground‑truth queries for evaluation
│   └── run_eval.py             # Reproducible Recall@10 / Precision@10 evaluation
...
### 🎯 Offline Evaluation (Production‑Style)

To complement the research‑phase NDCG, the system includes a **reproducible offline evaluation pipeline** (`benchmark/run_eval.py`) that measures **Recall@10** and **Precision@10** against a curated set of 5 movie titles with known relevant movies (`benchmark/test_queries.json`). The evaluation runs entirely without external API calls, giving fully deterministic results.
...
These numbers demonstrate that the semantic retrieval + hybrid ranking consistently surfaces all expected relevant movies within the top‑10, while maintaining strong precision. The evaluation script can be extended with more queries and graded relevance for NDCG.
...
**Live demo:** [https://huggingface.co/spaces/PavanBoorla/movie-mind](https://huggingface.co/spaces/PavanBoorla/movie-mind)
...
[GitHub](https://github.com/pavanramkalyanboorla-debug) · [LinkedIn](https://www.linkedin.com/in/pavan-ram-kalyan-boorla-0a3402405)
